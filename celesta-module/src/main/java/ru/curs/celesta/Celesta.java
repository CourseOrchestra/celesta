 ...
 */
public final class Celesta implements AutoCloseable, PyCelesta {
    ...
    private final ConcurrentHashMap<String, PySessionContext> sessions = new ConcurrentHashMap<>();
    ...
    public void login(String sessionId, String userId) {
        ...
        // нет старого.
        PySessionContext oldSession = sessions.get(sessionId);
        if (oldSession == null || !userId.equals(oldSession.getUserId())) {
            PySessionContext session = new PySessionContext(userId, sessionId);
    ...
    public void logout(String sessionId, boolean timeout) {
        PySessionContext sc = sessions.remove(sessionId);
        if (sc != null) {
