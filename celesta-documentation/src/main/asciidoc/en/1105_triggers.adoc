...
IMPORTANT: It often makes sence to use pre-triggers to perform certain action before the change is entered in the database.
Note that since "onPreInsert" trigger is executed before sending cursor contents to the database, values of fields posessing `DEFAULT` or `GETDATE()` properties are not filled in at the moment of its execution.
