...
{apidocs}ru/curs/celesta/dbutils/ReadOnlyTableCursor.html[ReadOnlyTableCursor]:: Is used to work with tables declared "WITH READ ONLY" option.
It does not add any own methods to `BasicCursor`.
...
The `skip` attribute indicates the number of rows skipped before returning (`skip = 0` — start from the very beginning), `limit` — maximum number of returned rows, where `limit = 0` indicates that *all* rows are returned.
...
filters records with city code starting with Latin M.
