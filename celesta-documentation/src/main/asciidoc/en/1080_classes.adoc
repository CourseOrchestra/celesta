...
* {lupa} *canRead(), canInsert(), canModify(), canDelete()* – returns a boolean value indicating the current session has rights to perform a corresponding operation.
...
In most actual cases cursor filtering by field value can be implemented using `setRange(...)` methods with two or three attributes filtering values by "field = value" condition or "field between value1 and value2" condition.
...
To be more exact, filter terms for text fields has the following syntax:
