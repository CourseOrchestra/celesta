...
A JUnit5 dependency must also be added to the test scope of the project (for examples see the https://junit.org/junit5/docs/current/user-guide/#dependency-metadata-junit-jupiter-samples[Junit5 Documentation]).

== Usage Example

The easiest way to use it is to add a {apidocs}ru/curs/celestaunit/CelestaTest.html[`@CelestaTest`] annotation to the test class and use {apidocs}ru/curs/celesta/CallContest.html[`CallContext`] parameters in tests:
...
In this way every test can receive an active `CallContext` as a parameter.
...
For example, in some cases it might help to disable reference integrity check to simplify adding test data to tables linked by foreign keys to other tables.
