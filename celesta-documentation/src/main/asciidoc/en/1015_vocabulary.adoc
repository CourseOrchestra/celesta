...
Simultaneous match of a version tag and a grain checksum is enough to continue operation without trying to upgrade the database structure. To simplify verification and ensure the algorythm transparency, the checksum consists of two values: the script file length (written down as a decimal integer) and its https://en.wikipedia.org/wiki/Cyclic_redundancy_check#CRC-32_algorithm[CRC32] (written down as eight hexadecimal integers).
...
Existance of the table indicates that Celesta is connected to the corresponding database, otherwise it will try to create one from scratch.
