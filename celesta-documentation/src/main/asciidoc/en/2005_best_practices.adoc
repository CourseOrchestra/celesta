= Best Code Writing Practices in Celesta
:lang: ru
:is-main-doc:
...
Besides, the cursors and the objects constituting its inner state are not threadsafe, so the cursor must not be stored as a dividable resource.
Cursor closure, loosing a reference to it and transaction completion must normally happen simultaneously.

Cursor life duration is limited by `CallContext` live duration it was created with.
...
Still in some situations the cursor is created in an iteration, it must be closed explicitly.

CallContext <<too_many_accessors_warning,does not allow to create more than 1023 cursors>> (otherwise it causes an error in the database).
If a cursor can be created in an iteration, if must be closed (in the next point the code will be improved):
...
The best solution is to design code where cursors are created in the beginning of the service method and then used repeatedly:
...
== Use the Limit for Fields in the Cursor

If the table being read has many fields, but only some of them are worked with, use the <<limit_columns,field range limit>> to speed it up.
...
For example, `orderBy` does not range entries at the moment the call happens, it only forms the cursor state to be used for forming SQL queries to call second and third category queries.
...
The `get(...)` method extracts a single from the database by its known primary key.
It is a reliably fast (because the primary key always has an index) and an often demanded operation in practice.
The `get(...)` method _does not account for_ existence on the cursor of any filters, so in rare cases it is necessary to check if an entry with a known key is in the range set by certain current cursor filters, a `navigate("=")` method must be used.
...
Counting all the entries at the database to determine if there are any at all is a bad idea.
...
== Use Ranging at the Database Level to Find Min./Max. Values

If the search can be done in the database itself, better do that instead of sending the whole entry set to the application to search using its resources:
...
*Do not use more complex filters if the simpler one is enough!*
...
[NOTE]
`cursor.setRange(cursor.COLUMNS.foo())` (no attributes) removes a filter for the “foo” field created earlier by `setRange` or `setFilter`.
...
`setRange` must be used whenever possible so Celesta can reuse JDBC `PreparedStatement`s to greatly increase performance during the iteration:
...
Main use for this method is to set conditions between fields, for example: `a >= b`.
In other cases `setRange`/`setFilter` should be applied.
...
Data query iteration is a common pattern.
In this case it is possible to significantly reduce the number of queries to the database and the increase speed by using the simplest cashing: there is no need to call `get` for the cursor if the requested value of the primary key already aligns with the one requested earlier:
...
This idea can be developed in a very efficient pattern: it is possible to drastically reduce the number of queries for `bar` entries to the database by ranging the `c` cursor by the `barId` field:
...
Many cursor methods have two versions: without a `try` prefix and with it.
...
Two groups of methods exists to move between entries in a cursor: navigation and iteration.

Navigation methods are `tryFirst()`, `next()`, `previous()` and `navigate(...)` which allow to move to a different entry according to specific rules.
...
Navigation methods send a single query to the database during each call so they should be utilized when only a single entry is needed.

Iteration methods send queries and open JDBC `ResultSet` used for further movement between entries.
