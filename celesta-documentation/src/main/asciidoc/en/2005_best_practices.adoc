...
Cursor closing, loosing a reference to it and transaction completion must normally happen simultaneously.

Cursor life cycle is enclosed into `CallContext` life cycle it was created with.
...
Querying data in iteration is a common pattern.
It is possible to significantly reduce the number of queries to the database and the increase speed by using the simplest caching: there is no need to call `get` for the cursor if the requested value of the primary key already matches the one requested earlier:
