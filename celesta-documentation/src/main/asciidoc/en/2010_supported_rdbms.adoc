...
Celesta solutions are easily transferrable between different supported RDBMS, but each of them have their own configuration features.
...
A problem with "uuid" type fields may occure when Celesta is used to access preexisting (and not created and updated in Celesta) database.
