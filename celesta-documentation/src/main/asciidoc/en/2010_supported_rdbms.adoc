...
The system supports MS SQL Server, Oracle, Postgre SQL, Firebird and H2 ensuring uniform work of any application written with Celesta with any of the supported RDBMS.
...
Schema in Oracle is somewhat different from those in other RDMBS and creating schemas is linked to creating new users, but in practice administrators of Oracle enterprise database servers cannot grant rights to create new users to application.
...
* Grain is a prefix in the table name spaced with an underscore symbol. 
Besides that, the limitation on the identifier length is active.
...
As of February 2020, Firebird 4.0 is in beta.
