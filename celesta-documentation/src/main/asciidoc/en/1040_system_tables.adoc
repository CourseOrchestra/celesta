...
Records are written here by the system while in <<profiling_mode,profiling mode>>.
...
* 2 – *error* — the last autoupgrade attempt failed, the `message` field contains the error message.
...
Special system role names are available: *reader* and *editor*, as well as roles in form of *<grain ID>.reader* and *<grain ID>.editor*.
