...
Just like Java has JavaDoc for object documentation or Python has a documenting constant available in the runtime, CelestaSQL has a special tool to document defined database objects: comments in a special format /** ... */ (two asterisks after the first slash, instead of a single asterisk in a standard comment).
...
Parser reads CelestaDoc and this information is available in metadata objects in the runtime by using the *getCelestaDoc()* method (see the <<Celesta_metadata,Celesta Metadata>> section).
