...
This is why Celesta code is executed in certain context determined by a  ({apidocs}ru/curs/celesta/CallContext.html[`CallContext`]) class instance.
...
So for the developmer it is enough to pass a not activated context to them.
Proxy object created by Spring framework around a service class instance will activate and close context and roll back database transactions if an uncatched exception is thrown.
...
Context closing and commits/rollbacks are performed by Celesta automatically 'under the hood'.

== Call Context Usage

Each method in the service layer using Celesta must have an argument of  {apidocs}ru/curs/celesta/CallContext.html[`ru.curs.celesta.CallContext`] type.
This call context is intended to be an argument for cursor constructors.
But also, a number of public methods and properties are available in `CallContext`, that are feasible to be used in a service method:

commit():: Commits the current transaction.
A need in this method occurs only in rare cases when data processing transaction should be split in several parts.
Usually it is not needed, as transactions are commited automatically at the end of the procedure.
