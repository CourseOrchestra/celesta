...
This means that MS SQLServer allows table names like this: [Long Table$Name]. Celesta, on the other hand, cannot support spaces and non-alphabetical symbols in IDs because a name of every Celesta table must be a name of a Java class and a name of every Celesta field must be a name of Java variable.
...
. *In addition, grain IDs cannot contain underscores*.
...
. The `START WITH` value does not upgrade for sequences created earlier even if changed in the grain file (because Oracle allows this operation only by deleting and recreating the sequence and Celesta does not allow stateful object deletion from the database).
...
It is used when structure of a certain table is changed in the database by other means and the system should not try to autosynch the table's structure with the definition given by CelestaSQL script.
...
Most functions not supported at the CelestaSQL level can be efficiently emulated with data access class API.
...
It is important to note that *term* inside a `SUM` statement must be a number.
...
Function are created with the following statement:
