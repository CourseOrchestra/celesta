...
The `UNIQUE` keyword lets you place a uniqueness constraint on a group of columns. Because different database engines handle nullable columns in unique indexes differently, Celesta permits creating a `UNIQUE` index only when every column in the index is defined with the `NOT NULL` constraint.
