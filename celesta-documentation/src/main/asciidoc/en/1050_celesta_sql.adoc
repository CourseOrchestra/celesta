...
== CelestaSQL Language to Determine Database Objects

WARNING: Scripts in CelestaSQL language must be encoded in UTF-8.

Celesta SQL language is used to write scripts for determining grains.
Script in CelestaSQL consists of statements
...
Standard comments can be used in any part of the text and CelestaDoc comments — only right before the grain, table, field or index definition.

== Object IDs in CelestaSQL

Object IDs are names of grains, tables, fields, indices, limitations and views, in Celesta their syntax is subject to a number of stringent limitations.

. In other RDBMS object IDs are allowed to contain spaces and non-alphabetical symbols if the names are encased in special symbols, like "[]" in MS SQL Server.
This means that MS SQLServer allows table names like this: [Long Table$Name]. Celesta, on the other hand, cannot support spaces and non-alphabetical symbols in IDs because names of every Celesta table must be names of Java classes and names of every Celesta field – names of Java variables.
So IDs of all named objects in CelestaSQL must at least meet the naming requirements for Java variables, i.e. *they must contain only lower- and uppercase Latin letters, numerals and underscores and must not start with numerals*.

. *Encasing Celesta IDs with quotation marks* in CelestaSQL scripts *is not allowed* at the syntax level since there is no practical need to do it (names never contain spaces).
...
. *IDs* in Celesta *are case-sensitive*, but it is impossible to create two tables with names different only by letter register.

. The *length* of every ID in Celesta *cannot exceed 30 characters*.

. *In addition, grain IDs (prefixes) cannot contain underscores*.
This is because combining grain names with underscores and other IDs often used for Celesta internal needs, this restriction helps to prevent possible ambiguities.

[[create_schema_statement]]
== CREATE SCHEMA (GRAIN) Statement

Every grain determination script must start with `CREATE SCHEMA`.
The following syntax is used (words `GRAIN` and `SCHEMA` are synonyms):

include::../synsql.adoc[tag="syn_create_schema"]

`WITH NO AUTOUPDATE` operation excludes the whole schema from the database autoupdate process.
It can be used when the schema structure is under external management, as the relevant <<celestasql_with_options,table option>>.

Version indication is mantadory to rule out involuntary database automatic downgrade when an older grain version launches with a more recent database version.
...
Every component of the version can be in bicomponent (and only this!) format and either has no prefix or has one containing only Latin uppercase letters and an underscore.
...
* "TITAN3.34,1.23" – the same version (tag sequence does not matter), autoupgrade takes place only if checksums do not align, no errors will occur
...
Versions are compared as digits with floating points.
...
. If the `MINVALUE` value is not indicated, it is automatically set equal to the `START WITH` value (by default 1).

. If the `MAXVALUE`, value is not indicated, it is automatically set equal to the `Long.MAX_VALUE` value.

. The `START WITH` value does not upgrade for sequences created earlier even if changed in the grain file (because Oracle allows this operation only by deleting and recreating the sequence and Celesta does not envisage stateful object deletion from the database).

. If the grain contains a table with a name, for example, `A`, it is not possible to create a sequence named `A_seq`, since the name is already reserved by the system.
...
In other words, the `CREATE TABLE` statement can list the definitions of fields, primary keys and foreign keys in any order in the brackets and options can be listed outside them.
In practice, first definitions of all fields follow each other, then go multipiece primary keys (single-field primary keys may be defined in the field itself) and multipiece foreign keys (if single-field, they may be defined in the field itself).
...
Syntax rules for foreign keys (*fk_rules*) can be found in the <<foreign_keys_statements,Foreign Rules>> section.
...
See also the <<RDBMS_peculiarities,Features of Using Supported RDBMS in Celesta>> section.
...
Here *<binary literal>* is a hexadecimal view of a byte sequence starting with 0x and not put in the quotation marks, for example: 0xFFAAFFAAFF.
...
*primary_key_definition* is a statement determining the composition of fields constituting primary key of the table.
...
* Foreign keys, either simple of composite, can refer only to primary keys and only in full. (Celesta SQL does not allow `UNIQUE` restrictions, so from two possibilities usually provided in RDBMS for foreign keys, it supports only refering to primary key).

* Field types must fully match (if the field is a string one, the length of the refering field must be exactly the same as the refered field length).

* Single set of columns cannot contain more than one foreign key definition (special case: there cannot be two foreign keys definied for the same column).
...
* `SET NULL` – setting `NULL` in links.
Naturally, this action is prohibited for `NOT NULL`-able fields.

* `CASCADE` – cascade field deletion/update.

At the moment of foreign key creation only the following can be referred to:
...
The only widely used actually important example of "looping" is when table refers to itself when organizing hierarchical list by parent–child principle.
...
* `WITH VERSION CHECK` — defalut mode: enabling entry version tracking to avoid losing updates (see the <<Lost_updates_protection,Lost Update Prevention>> section).
This option does not have to be indicated explicitly.

* `WITH NO VERSION CHECK` — disabling entry version tracking.
...
Used when table data comes from external sources and not entered by Celesta means or it is necessary to connect to a table related to a different application and so it is better not to make any changes to its data.
...
In addition, it is not necessary to specify primary keys to such tables.

* `NO AUTOUPDATE` option disconnects the table from database autoupdate and can be used along with other options.
It is used when structure of a certain table is changed in the database by other means and the system should not try to autosynch the table's structure with the description in CelestaSQL language.

[[create_index_statement]]
== CREATE INDEX Statement

Indices are used to speed up filtering by table field and are created with the following syntax statement:

include::../synsql.adoc[tag="syn_create_index"]

Names of indices within a grain must be unique.
All indices in Celesta allow repeating values.
...
* *query* SQL is a SQL query with the following syntax:
...
. `ORDER BY` is not supported since orderBy(...) method of the relevant cursor allows to filter the selection in a specific manner.
...
. `UNION` and `WITH` statements and inline queries are not supported.

Most functions not supported at the CelestaSQL level can be efficiently emulated in data access class API.
...
It is possible to conduct normal priority common arithmetical operations with `INT` and `REAL` type expressions: unary minus has the top priority, next goes multiplication and division ("*", "/"), still next goes adding and substracting ("+", "-").
It is possible to conduct concatenation operation "||" with `VARCHAR` type expressions.
Operation with fields of other types are not allowed.
...
It is important to note that *term* inside a `SUM` statement must be a digit.

include::../synsql.adoc[tag="syn_group_by"]

`GROUP BY` statement is special in Celesta-SQL because it is necessary to always list all not aggregated columns in the selection.
...
. If a `DATETIME` type column is involved in a `GROUP BY` expression, its values are rounded to days (hours, minutes and more accurate measurements are cut down).

. Only `SUM` and `COUNT` operations are available from the list of all aggregate operations.
...
They serve to grant read-only access to the data collected from one or several tables using a `SELECT` SQL query, taking into account transferred parameters.

Function are created with the following syntax statement:

include::../synsql.adoc[tag="syn_create_function"]

Parameters are declared with parameter name and type set and are separated with commas as follows:
...
Otherwise, syntax expression is similar to common view.
