...
Version indication is mandatory to rule out involuntary database automatic downgrade when an older grain version launches with a more recent database version.
...
* Field types must fully match (if the field is a string one, the length of the refering field must be exactly the same as the referred field length).

* Single set of columns cannot contain more than one foreign key definition (special case: there cannot be two foreign keys defined for the same column).
...
* `WITH VERSION CHECK` — default mode: enabling entry version tracking to avoid losing updates (see the <<Lost_updates_protection,Lost Update Prevention>> section).
