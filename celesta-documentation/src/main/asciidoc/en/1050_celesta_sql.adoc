...
Object IDs are names of grains, tables, fields, indices, limitations and views.
...
. *Encasing Celesta IDs with quotation marks* in CelestaSQL scripts *is not allowed* at the syntax level since there is no practical need to do that (names never contain spaces).
...
. *IDs* in Celesta *are case-sensitive*, but it is impossible to create two tables with names different only in letter register.
