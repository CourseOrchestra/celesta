...
|Defines the use of the foreign keys referetial integrity checks, when working with an H2 database in in-memory mode (for all other databases this parameter is ignored).
By default the referetial integrity checks are disabled when working with an H2 database in in-memory mode to simplify autotest creation.
|No
|`false`
