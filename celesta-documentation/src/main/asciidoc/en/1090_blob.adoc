...
. Unlike fields of other types, when a record is read with `get()`, `next()` or other similar methods, BLOB field values are not fetched from the tatabease and always assigned with a `null` instead.
...
Assigning a `null` value via a setter will not work, unlike with other field types, because the system will interpret it as if BLOB is not read from the batabase and `update()` will not change anything.
