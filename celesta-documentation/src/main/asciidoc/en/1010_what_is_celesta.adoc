 ...
 ** Postgre SQL,
 ** H2,
 ** Firebird (beta).image::{img}/640px-Duke2-2.png[width=300]
...
. Simplified *unit testing*: quick tests of code that interacts with database, based on H2 database in-memory operation mode.
. Automatically generated intermediary access layer for tables to create business logic based on the database structure.
. Application interaction with an existing database (applications integration via database).
