...
Опция `WITH NO AUTOUPDATE` исключает схему целиком из процесса автообновления базы данных. Как и соответствующая <<celestasql_with_options,опция таблицы>>, может быть использована в ситуации, когда структура схемы находится под внешним управлением.
...
* `WITH VERSION CHECK` — режим по умолчанию: включение механизма отслеживания версий записей для исключения потерянных обновлений (см. раздел <<Lost_updates_protection,«Защита от потерянных обновлений»>>). Указывать данную опцию явно не требуется.
...
. Не поддерживаются конструкции `UNION`, `WITH`, вложенные запросы. 
