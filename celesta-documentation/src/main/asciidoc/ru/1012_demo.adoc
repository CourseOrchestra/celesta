...
Если вам необходимо использовать како-то особенный пул соединений, или если вы желаете использовать  https://docs.oracle.com/javase/8/docs/api/javax/sql/DataSource.html[`DataSource`], предоставляемый фреймворком Spring, вы также можете это сделать. 

В самостоятельном приложении вы можете исопльзовать метод <<celesta_instantiation,`Celesta.createInstance`>>, подставив `DataSource` в качестве параметра.
