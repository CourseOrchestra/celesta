...
и так далее. У классов `Cursor` и `ViewCursor` имеется также перегруженная версия метода `create`, позволяющая создавать экземпляры объектов доступа к данным с <<limit_columns,ограничением по выборке столбцов>>.
