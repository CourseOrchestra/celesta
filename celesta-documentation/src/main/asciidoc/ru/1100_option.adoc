...
Чтобы объявить, что поле может принимать лишь значения из определённого списка, необходимо прописать свойство option в <<CelestaDoc,CelestaDoc>> целочисленного или текстового поля. Например, так:
