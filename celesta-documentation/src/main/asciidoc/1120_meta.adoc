...
Следует понимать, что хотя Celesta не будет перезаписывать .sql-файл, если в этом нет нужды (если метаданные не были изменены), при перезаписи содержимого .sql-файлов происходит необратимая потеря комментариев и форматирования, существовавших в них ранее. Сохранены в тексте будут только <<CelestaDoc,CelestaDoc>>-комментарии. Поэтому динамическим изменением следует пользоваться с осторожностью.
